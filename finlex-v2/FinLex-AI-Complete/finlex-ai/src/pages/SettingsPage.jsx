...
export default function SettingsPage() {
  ...
  const [co,   setCo]   = useState({ name:'', gstin:'', pan:'', tan:'', address:'', phone:'', email:'', business_type:'private_limited' })

  useEffect(() => {
    if (company) setCo({ name:company.name||'', gstin:company.gstin||'', pan:company.pan||'', tan:company.tan||'',
          ...
          {tab==='company' && (
            <div style={{ background:'var(--white)', borderRadius:18, padding:'28px 32px', border:'1px solid var(--gray-200)' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:24 }}>
                ...
                <F label="TAN (for TDS deductor)" icon={Hash}><input value={co.tan} onChange={e=>setCo({...co,tan:e.target.value.toUpperCase()})} style={{...INP,fontFamily:'var(--font-mono)',letterSpacing:'0.05em'}} placeholder="MUMB12345A" maxLength={10}/></F>
              </div>
