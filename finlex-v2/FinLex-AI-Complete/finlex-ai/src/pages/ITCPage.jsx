...
import { Upload, CheckCircle, AlertTriangle, XCircle, RefreshCw,
         TrendingUp, AlertCircle, Shield, FileText } from 'lucide-react'
  ...
  return (
    <div style={{ animation: 'fadeUp 0.5s ease' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 26, fontWeight: 700, color: 'var(--navy)', marginBottom: 2 }}>ITC Reconciliation</h1>
        <p style={{ color: 'var(--gray-600)', fontSize: 14 }}>Match your purchase register against GSTR-2B to verify ITC claims</p>
      </div>
      ...
      {register && (() => {
        ...
        const lost          = reconcile ? parseFloat(reconcile.summary?.ineligible_itc || 0) : null
        return (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:13, marginBottom:20 }}>
            {[
              { label:'Total ITC Available', value:`₹${totalITC.toLocaleString('en-IN')}`, sub:`From ${invoiceCount} purchase invoices`, Icon:TrendingUp, color:'#16a34a', bg:'#f0fdf4' },
              { label:'Purchase Value', value:`₹${totalPurchase.toLocaleString('en-IN')}`, sub:'Taxable amount excl. GST', Icon:FileText, color:'#3b82f6', bg:'#eff6ff' },
              { label: reconciled !== null ? 'Eligible ITC' : 'Reconciliation', value: reconciled !== null ? `₹${reconciled.toLocaleString('en-IN')}` : 'Not run yet', sub: reconciled !== null ? 'Matched with GSTR-2B' : 'Upload GSTR-2B to reconcile', Icon:CheckCircle, color: reconciled !== null ? '#16a34a' : '#ca8a04', bg: reconciled !== null ? '#f0fdf4' : '#fffbeb' },
              { label: lost !== null ? 'ITC at Risk' : 'GSTR-2B Status', value: lost !== null ? `₹${lost.toLocaleString('en-IN')}` : 'Pending', sub: lost !== null ? 'Mismatched / missing in 2B' : 'Upload JSON from GST portal', Icon: lost > 0 ? AlertCircle : Shield, color: lost > 0 ? '#dc2626' : '#6d28d9', bg: lost > 0 ? '#fef2f2' : '#f5f3ff' },
            ].map((s,i) => (
              <div key={i} style={{ background:'var(--white)', borderRadius:12, padding:'14px 16px', border:'1px solid var(--gray-200)', boxShadow:'var(--shadow-sm)' }}>
                ...
                <div style={{ fontFamily:'var(--font-display)', fontSize:17, fontWeight:700, color:'var(--navy)', marginBottom:2 }}>{s.value}</div>
