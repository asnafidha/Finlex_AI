  ...
  return (
    <div style={{ animation: 'fadeUp 0.4s ease' }}>
      ...
      {(actionItems.length > 0 || actionsLoading) && (
        <div style={{ background: 'var(--white)', borderRadius: 16, border: `1.5px solid ${criticalCount > 0 ? '#fecaca' : '#fde68a'}`, marginBottom: 24, overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
          ...
          {deadlines.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {deadlines.map((d, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 8, background: isOverdue ? '#fef2f2' : 'var(--gray-100)', border: `1px solid ${isOverdue ? '#fecaca' : 'var(--gray-200)'}` }}>
