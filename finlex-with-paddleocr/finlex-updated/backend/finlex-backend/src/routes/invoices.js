...
// GET /api/invoices?company_id=xxx&invoice_type=sale
...
// PATCH /api/invoices/:id/cancel — cancel invoice + reverse journal entry
  ...
  try {
    ...
    if (jeRows.length > 0) {
      ...
      const { rows: countRows } = await client.query(
        'SELECT COUNT(*) FROM journal_entries WHERE company_id=$1', [invoice.company_id]
      )
    ...
    await client.query(
      `INSERT INTO audit_log(company_id,user_id,action,table_name,record_id,new_values)
       VALUES($1,$2,'INVOICE_CANCELLED','invoices',$3,$4)`,
      [invoice.company_id, req.user.id, invoice.id,
       JSON.stringify({ invoice_number: invoice.invoice_number, party: invoice.party_name, amount: invoice.total_amount })]
    )

    await client.query('COMMIT')
    res.json({ message: `Invoice ${invoice.invoice_number} cancelled and journal reversed`, reversal_entry: jeRows.length > 0 ? 'created' : 'no_journal_found' })
  } catch (err) {
    await client.query('ROLLBACK')
    res.status(500).json({ error: err.message })
  } finally { client.release() }
...
async function createJournal(client, companyId, invoice, cgst, sgst, igst, userId, tdsAmount=0, tdsSection=null) {
  ...
  if (isSale) {
    ...
    if (tdsAmount > 0) {
      // TDS deducted at source: AP is reduced, TDS Payable is credited
